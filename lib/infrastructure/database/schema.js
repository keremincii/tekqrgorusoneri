...
export const sikayetler = pgTable('sikayetler', {
  ...
  /**
   * BAŞVURU TÜRÜ — bu ürünün TEK sınıflandırma ekseni: 'sikayet' | 'gorus' | 'oneri'
   * (constants.BasvuruTurleri). Vatandaşa kategori sorulmaz; yalnız türünü seçip yazar.
   *
   * Whitelist ÜÇ katmanda uygulanır: form (BasvuruTurleri) → servis
   * (basvuruTuruGecerliMi) → DB (`sikayetler_tur_check` CHECK kısıtı). En alttaki
   * katman kasıtlıdır: elle SQL ya da bakım script'iyle girmiş tutarsız bir tür,
   * uygulama katmanı hiç çalışmasa bile veritabanında kesilir.
   */
  tur: varchar('tur', { length: 20 }).default('sikayet').notNull(),

  /**
   * İlişkili QR noktası (Foreign Key). TEK QR ürününde bu her zaman aynı satırdır
   * (kent meydanı) — kolon, şema bütünlüğü (NOT NULL + FK) ve HMAC imza kapısının
   * bağlandığı kayıt olarak durur; vatandaşa bir konum seçtirilmez.
   */
  sokakId: uuid('sokak_id').references(() => sokaklar.id).notNull(),

  /**
   * LEGACY (sokak-QR modeli) — ARTIK YAZILMAZ. Çok QR'lı üründe vatandaş, okuttuğu
   * QR'ın sokağı yerine kayıtlı olmayan bir sokak bildirebiliyordu ve adı buraya
   * yazılıyordu. Tek QR'da seçilecek sokak olmadığı için bu yol kapandı; kolon eski
   * kayıtları bozmamak için durur ve listeleme sorgusundaki coalesce ile okunmaya
   * devam eder (yeni kayıtlarda daima NULL).
   */
  ...
  /**
   * LEGACY — ARTIK YAZILMAZ (migration 0001 ile NOT NULL kaldırıldı).
   * Çok QR'lı üründe vatandaş 7 sabit kategoriden birini seçiyor, kategori
   * `birim_kategoriler` üzerinden bir birime yönleniyordu. Tek QR ürününde vatandaşa
   * kategori SORULMAZ (yerine `tur`), dolayısıyla otomatik saha yönlendirmesi de
   * yoktur — iş dağıtımı yönetimin ATAMA kararıdır. Kolon eski kayıtlar için durur.
   */
  kategori: varchar('kategori', { length: 50 }),

  /** Başvuru metni (vatandaşın yazdığı; bu üründe TEK içerik alanı, zorunlu). */
  ...
  // Panel listesi: tenant + durum + tarihe göre sıralı listeleme
  index('sikayetler_tenant_durum_tarih_idx').on(t.tenantId, t.durum, t.olusturmaTarihi),
  // Panel sekmeleri (tür bazlı liste + rozet sayacı) — migration 0000 sonu
  index('sikayetler_tenant_tur_durum_tarih_idx').on(t.tenantId, t.tur, t.durum, t.olusturmaTarihi),
...
// ==========================================
// Saha personelini gruplamak içindir (ör. "Temizlik İşleri", "Fen İşleri").
//
// ARTIK BİR YÖNLENDİRME KURALI DEĞİLDİR: kategori ekseni kalktığı için "hangi
// kategori hangi birime düşer" eşleşmesi (`birim_kategoriler`) ve ona dayalı otomatik
// bildirim de kalktı (migration 0001). Birim bugün yalnız panelde "kime atayayım?"
// sorusunu kolaylaştıran bir gruplamadır; iş dağıtımı yönetimin ATAMA kararıdır.
...
// NOT — KALDIRILAN TABLO: `birim_kategoriler` (kategori ↔ birim yönlendirme
// eşleşmesi) migration 0001 ile DÜŞÜRÜLDÜ. Kategori ekseni olmayan bir üründe
// eşleşmenin sol tarafı yoktur; tabloyu şemada tutmak "otomatik yönlendirme hâlâ var"
// yanılgısı üretirdi. Bildirim kuralı artık tek cümle: yeni başvuru → başkan/yardımcıya
// bilgi, saha personeline yalnız ATAMA ile.

/**
 * Engelli kimlikler (kara liste) — TENANT'TAN BAĞIMSIZ, TÜM BELEDİYELERİ KAPSAR.
 *
 * Operatör (moderasyon botundan ya da panelden) bir kimliği engellediğinde, o
 * kimlik_hash bu tek listeye eklenir ve bu dağıtımdaki (aynı veritabanını paylaşan)
 * HİÇBİR belediyeye bir daha SMS kodu / başvuru gönderemez — engellendiği belediye
 * neresi olursa olsun.
 *
 * NEDEN GLOBAL (migration 0002 ile tenant_id kaldırıldı): kimlik_hash bu dağıtımın
 * TEK bir HMAC_SECRET'ıyla türetilir; aynı telefon, dağıtımdaki HANGİ belediyeden
 * başvursa AYNI hash'i üretir. Troll/kötüye kullanım bir belediyede engellenip
 * komşu belediyede aynı numarayla devam edebilseydi, engel adı üstünde caydırıcı
 * olmazdı. (Bu kapsam yalnız BU dağıtım içindir: HMAC_SECRET'ı farklı, AYRI bir
 * dağıtımdaki [ör. başka bir sunucudaki] belediyeye uzanmaz — oradaki hash matematik
 * olarak bambaşka bir değerdir, sırrı paylaşmadan eşleştirilemez.)
 *
 * Ham telefon/isim SAKLANMAZ; engelleme yalnız hash iledir (KVKK veri minimizasyonu).
 */
export const engelliKimlikler = pgTable('engelli_kimlikler', {
  ...
  uniqueIndex('engelli_kimlikler_hash_uniq').on(t.kimlikHash),
]);
