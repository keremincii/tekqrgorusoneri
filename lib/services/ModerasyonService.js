import {
  SikayetKategorileri, SikayetDurumu, GuvenlikSabitleri,
} from '@/lib/utils/constants.js';

/**
 * Kategori id → "ikon etiket" (Single Source of Truth: constants).
 * (TelegramService'teki eşiyle aynı kural — biri değişirse diğeri de değişmeli.)
 */
...
/**
 * Kategori satırı. Tanınmayan bir id (eski/elle girilmiş kayıt) satırı hiç bastırmaz —
 * "📌 Şikayet" gibi bilgisiz bir satır operatöre bir şey söylemez.
 */
function kategoriSatiri(sikayet) {
  return KATEGORI_TABLOSU[sikayet.kategori] ? kategoriEtiket(sikayet.kategori) : null;
}
...
export class ModerasyonService {
   ...
   */
  constructor({ telegramClient, chatId, sikayetRepo, sokakRepo, tenantRepo = null, telegramServiceGetir }) {
  ...
  _gorunmezlikNotu() {
    return 'Vatandaşa normal başarı mesajı gösterildi. Bu şikayet haritaya ve sahaya <b>düşmedi</b>.';
  }

  /** Şikayetin gösterilecek sokak adı (serbest metin varsa o, yoksa kayıtlı sokak). */
  async _sokakAdi(sikayet) {
    const serbest = sikayet.bildirilenSokakAdi != null && String(sikayet.bildirilenSokakAdi).trim() !== '';
    if (serbest) return String(sikayet.bildirilenSokakAdi).trim();
    const sokak = await this.sokakRepo.idIleGetir(sikayet.sokakId, sikayet.tenantId).catch(() => null);
    return sokak?.sokakAdi || 'Konum bilinmiyor';
  }
  ...
  async kufurBildir(sikayet, { eslesme, tur: kufurTuru } = {}) {
    ...
    try {
      const sokakAdi = await this._sokakAdi(sikayet);
      const kategori = kategoriSatiri(sikayet);
      ...
      const ustBilgi = [
        ...(sokakAdi ? [`<b>${htmlKacis(sokakAdi)}</b>`] : []),
        ...(kategori ? [kategori] : []),
      ];
      const metin = [
        `<b>${filtreBasligi}</b>`,
        ...
        htmlKacis(String(sikayet.aciklama || '(açıklama yok)').slice(0, 800)),
        '',
  ...
  async _bekleyenleriListele(chatId) {
    ...
    if (!bekleyenler.length) {
      await this.tg.sendMessage(chatId, '✅ Onay bekleyen şikayet yok.');
      return;
    }
    ...
    for (const s of bekleyenler) {
      ...
      const sokakSatiri = s.sokakAdi || 'Konum bilinmiyor';
      const kategori = kategoriSatiri(s);
      const metin = [
        ...(belediye ? [belediye] : []),
        `<b>${htmlKacis(sokakSatiri)}</b>`,
        ...(kategori ? [kategori] : []),
        '',
        htmlKacis(String(s.aciklama || '(açıklama yok)').slice(0, 800)),
        '',
  ...
  async _callbackIsle(cbq) {
    ...
    // Normal akış: TÜRÜN bildirim kuralına göre (şikayet/talep → kategorinin birimi,
    // teşekkür → seçilen birim, öneri → yalnız yönetim) bildirim. En iyi çaba —
    // patlarsa durum değişikliği yine geçerlidir.
    await this.telegramServiceGetir().yeniSikayetBildir(guncel || sikayet).catch((e) =>
   ...
   * kara listeye ekler → o numara bir daha SMS kodu isteyemez ve HİÇBİR TÜRDE başvuru
   * gönderemez. Ham telefon GEREKMEZ; hash zaten kayıtta durur.
   *
  ...
  async _engelleIsle(cbq, sikayet, tenantId) {
    ...
    await this.sikayetRepo.engelle(tenantId, kimlikHash, 'moderasyon-botu');
