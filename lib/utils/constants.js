 ...
 * Yeni kategori eklemek için sadece bu listeye eklemeniz yeterli.
 * Frontend, API ve validator otomatik olarak bu listeden okur (Single Source of Truth).
 */
export const SikayetKategorileri = Object.freeze([
  { id: 'cop-temizlik', etiket: 'Çöp / Temizlik', ikon: '🗑️' },
  { id: 'asfalt-yol', etiket: 'Asfalt / Yol Bozukluğu', ikon: '🚧' },
  { id: 'su-kanalizasyon', etiket: 'Su / Kanalizasyon', ikon: '💧' },
  { id: 'aydinlatma-elektrik', etiket: 'Aydınlatma / Elektrik', ikon: '💡' },
  { id: 'otopark-trafik', etiket: 'Otopark / Trafik', ikon: '🚗' },
  { id: 'hayvan-sorunu', etiket: 'Hayvan Sorunu', ikon: '🐕' },
  // NOT: id 'gurultu-cevre' geriye dönük uyum için AYNEN kalır (mevcut şikayetler bu
  // id'yi taşır; değiştirmek onları öksüz bırakır). Yalnız GÖRÜNEN etiket düzeltildi:
  // "Çevre Kirliliği" kaldırıldı çünkü vatandaş çöp/atığı yanlışlıkla buradan seçiyordu.
  { id: 'gurultu-cevre', etiket: 'Gürültü / Ses', ikon: '🔊' },
]);
...
/**
 * ===================== HARİTA PİN RENGİ: BEKLEME SÜRESİ =====================
 * Eskiden pin rengi o noktadaki AÇIK KAYIT SAYISINA göre kızarıyordu. Bu yanıltıcıydı:
 * kalabalık bir cadde, hepsi bugün açılmış 8 kayıtla "acil kırmızı" görünürken, tek
 * başına 3 haftadır bekleyen bir şikayet sakin sarı kalıyordu. Oysa yöneticinin haritada
 * araması gereken şey YIĞILMA değil İHMAL'dir. Renk artık tek bir soruyu yanıtlar:
 * "bu nokta ne kadar süredir bekliyor?"
 *
 * Bir noktada birden çok açık kayıt varsa EN ESKİSİ esas alınır (en kötü durum) — pin,
 * o noktanın en çok mağdur olan işini temsil etmelidir.
 *
 * Adet bilgisi kaybolmaz: pin ÜZERİNDEKİ sayı ve pin BOYUTU hâlâ açık kayıt sayısıdır.
 */
export const HaritaYasKademeleri = Object.freeze([
...
/** Sonuçlanmış kayıtların pin rengi (yalnız "çözülenleri göster" açıkken çizilir). */
export const HARITA_COZULDU_RENGI = '#34d399';
 ...
 */
export function haritaYasKademesi(gun) {
  const g = Number.isFinite(gun) ? gun : 0;
  for (let i = HaritaYasKademeleri.length - 1; i >= 0; i--) {
    if (g >= HaritaYasKademeleri[i].enAzGun) return HaritaYasKademeleri[i];
  }
  return HaritaYasKademeleri[0];
}
 ...
 * - personel: saha ekibi; bir birime bağlıdır, yalnızca kendi biriminin kategorisindeki
 *   şikayetlerin bildirimini alır ve çözebilir.
 * - baskan / baskan_yardimcisi: birime bağlı DEĞİL; HER yeni şikayetin ve HER çözümün
 *   bilgi bildirimini alır (Telegram).
 */
...
export const KvkkSabitleri = Object.freeze({
   ...
   * sürüm '2026-08-v15s'tir.
   */
  AYDINLATMA_SURUMU: '2026-08-v15',
...
export function saklamaTablosu(tenant) {
  ...
  satirlar.push(
    ...
    {
      veri: 'Başvuru içeriği (konu, açıklama)',
...
export const UygulamaBilgileri = Object.freeze({
  AD: `${process.env.NEXT_PUBLIC_BELEDIYE_ADI || 'Gülşehir Belediyesi'} Şikayet Sistemi`,
  VERSIYON: '1.0.0',
});
