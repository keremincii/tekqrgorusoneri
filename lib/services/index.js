...
import { getRedis, getRedisAbone } from '@/lib/infrastructure/redis/RedisClient.js';
import { BellekOlayYayini } from '@/lib/infrastructure/events/BellekOlayYayini.js';
import { RedisOlayYayini } from '@/lib/infrastructure/events/RedisOlayYayini.js';
import { BasvuruAkisServisi } from '@/lib/services/BasvuruAkisServisi.js';
...
/**
 * OLAY YAYINI SİNGLETON'I — `globalThis` üzerinde.
 *
 * Diğer servisler modül düzeyinde bir değişkende tutulabilir; olay yayını TUTULAMAZ.
 * Geliştirmede Next.js modülleri sıcak yeniden yükler (HMR): modül düzeyindeki
 * değişken sıfırlanır, YENİ bir yayın nesnesi doğar ve o ana kadar açık olan SSE
 * bağlantıları ESKİ nesneyi dinlemeye devam eder → yayınlar hiçbir panele ulaşmaz
 * ("çalışıyordu, sonra sessizce durdu" tipi hata). globalThis, modül yeniden
 * yüklense de aynı nesneyi verir.
 */
const OLAY_YAYINI_ANAHTARI = Symbol.for('tekqr.olayYayini');

// Singleton instance'lar (Serverless'ta her cold start'ta yeniden oluşturulur)
let _basvuruAkisServisi = null;
...
/**
 * Canlı olay yayınını döndürür (başkan panelinin SSE akışı).
 *
 * Seçim: Redis varsa kopyalar-arası (pub/sub) yayın, yoksa süreç içi yayın. İkisi de
 * IOlayYayini sözleşmesini uygular → çağıran hangisi olduğunu bilmez.
 * @returns {import('@/lib/domain/interfaces/IOlayYayini.js').IOlayYayini}
 */
export function getOlayYayini() {
  const mevcut = globalThis[OLAY_YAYINI_ANAHTARI];
  if (mevcut) return mevcut;

  const yayinci = getRedis();
  const abone = yayinci ? getRedisAbone() : null;
  // Abone bağlantısı kurulamadıysa Redis'li yola GİRMEYİZ: yayın gider ama kimse
  // dinlemez, yani olay sessizce kaybolurdu. Süreç içi yayın en azından bu
  // konteynerdeki panelleri besler (tek konteynerli dağıtımda tam işlevsel).
  const yayin = yayinci && abone
    ? new RedisOlayYayini(yayinci, abone)
    : new BellekOlayYayini();

  globalThis[OLAY_YAYINI_ANAHTARI] = yayin;
  return yayin;
}

/** Panelin canlı akışını besleyen servisi döndürür. */
export function getBasvuruAkisServisi() {
  if (!_basvuruAkisServisi) {
    _basvuruAkisServisi = new BasvuruAkisServisi({
      olayYayini: getOlayYayini(),
      sikayetRepo: new SikayetRepository(),
    });
  }
  return _basvuruAkisServisi;
}
...
/**
 * Telegram servisini döndürür (saha ekibi botu — atama bildirimi, yönetime yeni
 * başvuru bilgisi, webhook). Tek bot tüm belediyelere hizmet eder; tenant, bağlanan
 ...
 *
 * `akisServisi` LAZY bir fabrika olarak geçilir: Telegram'dan gelen "Çözüldü" açık
 * panellere de yansımalı, ama iki servis birbirini modül düzeyinde import etmemeli.
 */
export function getTelegramService() {
  if (!_telegramService) {
    _telegramService = new TelegramService({
      ...
      akisServisi: getBasvuruAkisServisi,
    });
  }
...
export function getModerasyonService() {
  if (!_moderasyonService) {
    _moderasyonService = new ModerasyonService({
      ...
      // Onaylanan kayıt görünür hâle gelir → açık panellere anında düşsün.
      akisServisi: getBasvuruAkisServisi,
    });
  }
