...
export const KvkkSabitleri = Object.freeze({
   ...
   * v17: ONAY KUTUSU TEKE İNDİ. v13'te AYRILAN iki kutu (aydınlatma + yurt dışı
   *      aktarıma açık rıza) tek kutuda birleştirildi:
   *        "Aydınlatma Metni'ni okudum, anladım ve kabul ediyorum."
   *      METNİN KENDİSİ (/kvkk) DEĞİŞMEDİ — yurt dışı aktarımı anlatan bölümler
   *      eksiksiz duruyor. Değişen yalnız formdaki onay arayüzüdür; sürüm, hangi
   *      ONAY BİÇİMİYLE kayıt alındığını ayırt edebilmek için artırıldı (aynı metne
   *      v16'da iki kutuyla, v17'de tek kutuyla onay verilmiştir).
   *
   *      ⚠ BU BİR ÜRÜN SAHİBİ KARARIDIR, hukuki bir iyileştirme DEĞİLDİR. Kayda
   *      geçsin: Kurul'un yerleşik görüşü aydınlatma ile açık rızanın AYRI alınması
   *      yönündedir ve v13'te tam da bu sebeple ayrılmışlardı. Bu üründe yurt dışı
   *      aktarımın (Almanya'daki sunucu, Telegram, Turnstile, R2) tek hukuki
   *      dayanağı açık rızadır; rıza sakat sayılırsa aktarım dayanaksız kalır.
   *      KALICI ÇÖZÜM: KVKK m.9 uyarınca standart sözleşmeyle uygun güvence sağlanıp
   *      Kurum'a bildirilmesi. O yapıldığında aktarım rızadan çıkar, bu kutu da
   *      hukuken saf bir "aydınlatma teyidi"ne dönüşür ve buradaki risk kapanır.
   * v18: SAĞLAYICI ADLARI METİNDEN ÇIKARILDI (v14'ün KISMEN GERİ ALINMASI). Metinde
   *      artık "Telegram", "Cloudflare", "Cloudflare R2" ve "netcup GmbH" adları
   *      geçmiyor; yerlerine yalnızca hizmetin niteliği ("bir mesajlaşma servisi",
   *      "yurt dışı merkezli bir bulut depolama hizmeti", "saldırı koruması sağlayan
   *      bir altyapı sağlayıcısı", "bir sağlayıcı") yazıldı. BARINDIRMA_ULKE
   *      (Almanya) bilinçli olarak KORUNDU — AB mevzuatına tabi olma iddiası bu
   *      bilgiye dayanıyor, o cümle ülke adı olmadan anlamsız kalırdı. VERI_ISLEYEN
   *      (işleyen şirketin kendi adı) de KORUNDU — bu KVKK m.10 gereği zaten
   *      açıklanması zorunlu veri işleyen kimliğidir, üçüncü taraf altyapı sağlayıcısı
   *      değildir.
   *
   *      ⚠ BU DA BİR ÜRÜN SAHİBİ KARARIDIR, hukuki bir iyileştirme DEĞİLDİR — v14'ün
   *      TAM TERSİ yönde. v14, açık rızanın "bilgilendirilmiş" sayılabilmesi için
   *      alıcının KİM olduğunun yazılması gerektiği gerekçesiyle sağlayıcıları isimle
   *      yazmıştı; bu adım o gerekçeyi feda ederek, kamuya açık sayfada teknik
   *      altyapının (hangi mesajlaşma servisi, hangi bulut sağlayıcı, hangi CDN)
   *      isimle ifşa edilmemesini önceliklendirir. Denetimde "alıcı belirsiz" itirazı
   *      gelirse BARINDIRMA_SAGLAYICI ve TELEGRAM benzeri sabitler hâlâ kod
   *      tabanında durur (bkz. aşağıdaki sabitler) — istenirse metne geri eklenebilir.
   * v19: METİN KENDİ FORMUNU YANLIŞ ANLATIYORDU (düzeltme). v17'de onay kutusu teke
   *      indirildi ama metnin son paragrafı ile aktarım bölümündeki cümle v13'ten
   *      kalma hâliyle "formda İKİ AYRI onay sunulur" / "formdaki AYRI açık rıza
   *      kutusu" demeye devam ediyordu. Vatandaş metni okuyup forma dönünce tek kutu
   *      görüyordu: aydınlatma metninin, rızanın nasıl alındığını yanlış tarif etmesi
   *      —üstelik bu rıza yurt dışı aktarımın TEK dayanağıyken— düzeltilmesi gereken
   *      bir tutarsızlıktı. İki cümle de tek kutuya göre yeniden yazıldı.
   *
   *      Ayrıca JSX BOŞLUK KUSURU giderildi: kapanan <strong> etiketinden sonraki
   *      boşluk render'da yutulduğu için metin dokuz yerde kelimeleri birleştirerek
   *      çıkıyordu ("30 güniçinde", "otomatik olarakuygulanır", "iki ayrı onaysunulur"
   *      …). Hepsine açık {' '} kondu. ANLAM DEĞİŞMEDİ, yalnız okunabilirlik.
   *      DERS: bu dosyada <strong>/<a> sonrasına düz boşluk yazma — {' '} kullan.
   *
   *      KAPSAM DEĞİŞMEDİ; sürüm yine de arttı çünkü vatandaşa GÖSTERİLEN metin
   *      değişti ve kayıttaki `kvkk_metin_surumu` hangi metnin gösterildiğini
   ...
   * sürüm '2026-08-v19s'tir.
   */
  AYDINLATMA_SURUMU: '2026-08-v19',

  /**
   * Aydınlatma metninde İSİMLE anılan taraflar (v14; v18'de kapsam daraltıldı).
   *
   ...
   *
   * v18 NOTU: BARINDIRMA_SAGLAYICI artık /kvkk sayfasında İSİMLE render edilmiyor
   * (yalnızca BARINDIRMA_ULKE render edilir) — sabit yine de burada tutulur, çünkü
   * operasyonel/iç dokümantasyon amaçlı kullanılabilir ve gerçek sağlayıcı adının tek
   * kaynağı budur.
   *
